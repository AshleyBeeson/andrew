...
database is localhost/projectdb currently

2. karma. 
./node_modules/.bin/karma start karma.config.js
or npm test
...
however. Remember: Tests are served through webpack.config.js !!!! 

3. Mongo database setup! 
-open POSTMAN. send a put request to database to force generate new db, and collection. Then use Robo 3T to manually add the json files.
-Else use C:/mongodb/bin/mongoimport.exe --database projectdb --collection bugs --file 'path-to-bugImport.json' --jsonArray
